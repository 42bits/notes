...
if not ngx.config or not ngx.config.ngx_lua_version or ngx.config.ngx_lua_version < 9003 then
    is_exiting = function()
        return false
    end
    ...
    ngx_log(
        CRIT,
        "We strongly recommend you to update your ngx_lua module to " ..
            "0.9.3 or above. lua-resty-logger-socket will lose some log " ..
                "messages when Nginx reloads if it works with ngx_lua module " .. "below 0.9.3"
    )
else
...
local function _connect()
    ...
    connecting = false
    if not connected then
        return nil, "try to connect to the log server failed after " .. max_retry_times .. " retries: " .. err
    end
...
local function _prepare_stream_buffer()
    ...
    if counter > max_buffer_reuse then
        ...
        if debug then
            ngx_log(
                DEBUG,
                "log buffer reuse limit (" .. max_buffer_reuse .. ') reached, create a new "log_buffer_data"'
            )
        end
    end
end
...
function _M.init(user_config)
    ...
    if not (host and port) and not path then
        return nil, 'no logging server configured. "host"/"port" or ' .. '"path" is required.'
    end
